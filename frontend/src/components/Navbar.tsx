    ...
    >
      <div className="container mx-auto px-4 md:px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          {/* Desktop Navigation */}
          <nav className="hidden md:flex items-center space-x-8  max-w-full overflow-x-hidden">
            {(isAuthenticated ? authenticatedNavLinks : navLinks).map((link, index) => (
              <Link
                key={index}
                to={link.path}
                className={`font-medium transition-colors hover:text-primary-600 dark:hover:text-primary-400 ${
                  location.pathname === link.path
                    ...
                    : location.pathname === '/jobs' && !scrolled
