...
import { FileText, Upload, Clipboard, Plus, Github, Linkedin, CheckCircle, File, Award, AlertCircle, Loader2, Check, LogOut, Eye, EyeOff } from 'lucide-react';
...
import ATSScoreHistory from '../components/ATSScoreHistory';
...
const DashboardPage = () => {
  ...
  // New: After OAuth, user is redirected back. Fetch updated profile.
  useEffect(() => {
    const urlParams = new URLSearchParams(window.location.search);
    if (urlParams.get('linkedin') === 'success') {
      // Fetch updated profile using AuthContext
      refreshProfile().then(() => {
  ...
  const handleGitHubDisconnect = async () => {
    ...
    try {
      ...
      if (response.data?.success) {
        ...
        // Update transformed resume URLs if user has a transformed resume
        try {
          await api.updateTransformedResumeUrls();
        } catch (urlError) {
          console.warn('Failed to update transformed resume URLs:', urlError);
  ...
  const handleLinkedInDisconnect = async () => {
    ...
    try {
      ...
      if (response.data?.success) {
        ...
        toast.success('LinkedIn profile disconnected successfully!');
        ...
        toast.error(response.data?.message || response.error || 'Failed to disconnect LinkedIn profile');
      }
    } catch (error) {
            ...
            >
              <div className="p-6">
                ...
                <nav className="space-y-1">
                  ...
                  {/* ATS Score History moved to component below */}
                  {/* <button
              ...
              {/* Upload and Create Buttons Section */}
              ...
              <ATSScoreHistory key={uploadSuccess ? Date.now() : 'static'} />
            </motion.div>
          </div>
              ...
              <div className="p-6 border-b border-gray-200 dark:border-gray-700">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    {/* Update URLs button for existing users */}
                    {(githubConnected || linkedInConnected) && user?.transformedResume && (
                      <Button 
                        variant="outline" 
                        size="sm"
                        onClick={async () => {
                          try {
                            await api.updateTransformedResumeUrls();
                            toast.success('Resume URLs updated successfully!');
                          } catch (error) {
                            console.error('Error updating URLs:', error);
                            toast.error('Failed to update resume URLs');
                          }
                        }}
                      >
                        Update URLs
